...
int TestReduceSum(
    ...
    const unsigned lenOutputUdt = dim0*dim1Padded;
    ...
    UnpadTensor<CONFIG_DTYPE>(hostOutputTn, hostUDT, dim0, dim1Padded, dim1);
    bool rslt = true;

    for(unsigned d0=0; d0<dim0; d0++){ 
        for(unsigned d1=0; d1<dim1; d1++){ 
            unsigned indxCpu = d0*dim1+d1;
